...
class Roomba(Agent):
    def __init__(self, unique_id, model, pos):
        ...
        self.pos = pos
        self.home_charger = pos  # Remember starting position (charging station)
    ...
    def move(self, new_pos):
        ...
        if not (0 <= new_pos[0] < self.model.grid.width and 0 <= new_pos[1] < self.model.grid.height):
            return False
    ...
    def get_possible_moves(self):
        ...
        for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:  # up, down, right, left
            ...
            # Check for obstacles
            cell_contents = self.model.grid.get_cell_list_contents([new_pos])
            is_obstacle = any(hasattr(content, 'state') and content.state == "obstacle" 
                            for content in cell_contents)
